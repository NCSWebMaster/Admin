// NCS OnePlace — pages/admissions/hub.js
// Admissions Hub: card-grid pipeline view by status/school year + Reports summary.
...
const els = {
  ...
  viewPipelineBtn: document.getElementById('view-pipeline-btn'),
...
function renderPipeline() {
  ...
  els.stageGrid.querySelectorAll('.stage-card').forEach(btn => {
    btn.addEventListener('click', () => { activeTab = btn.dataset.tab; expandedId = null; renderPipeline(); });
  });
...
function renderReports() {
  const nonDeleted = nonDeletedRows();
  const applied = nonDeleted.length;
  const accepted = nonDeleted.filter(r => r.status === 'accepted').length;
  const waitlisted = nonDeleted.filter(r => r.status === 'waitlisted').length;
  const declined = nonDeleted.filter(r => r.status === 'declined').length;
  const abandoned = nonDeleted.filter(r => r.status === 'code_sent' || r.status === 'in_progress').length;
...
els.viewPipelineBtn.addEventListener('click', () => {
  currentView = 'pipeline';
  els.viewPipelineBtn.classList.add('active');
  els.viewReportsBtn.classList.remove('active');
  els.pipelineView.style.display = 'block';
  els.reportsView.style.display = 'none';
  renderAll();
});

els.viewReportsBtn.addEventListener('click', () => {
  currentView = 'reports';
  els.viewReportsBtn.classList.add('active');
  els.viewPipelineBtn.classList.remove('active');
  els.reportsView.style.display = 'block';
  els.pipelineView.style.display = 'none';
  renderAll();
});
